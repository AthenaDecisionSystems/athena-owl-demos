...
def build_or_get_loan_client_repo():
    global _insurance_client
    if _loan_client:
        config = get_config()
        module_path, class_name = config.app_client_repository.rsplit('.',1)
...
def get_client_by_id(id: int) -> dict:
    ...
    return build_or_get_loan_client_repo().get_client_json(id)

def get_client_by_name(name: str) -> dict:
    """get client information given his or her name"""
    return build_or_get_loan_client_repo().get_client_by_name(name)
